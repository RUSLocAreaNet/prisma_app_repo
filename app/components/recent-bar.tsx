...
import { KudoStyle, Profile, Kudo } from '@prisma/client'
import { UserCircle } from './user-circle'
import { emojiMap } from '~/utils/constants'

interface KudoWithRecipientAndStyle extends Kudo {
  profile: Profile,
  style: KudoStyle
}

export function RecentBar({ kudos }: { kudos: KudoWithRecipientAndStyle[] }) {
    console.log("--------------------RECENT KUDOS-----------------")
    console.log(kudos)
  return (
    <div className="w-1/5 border-l-4 border-l-yellow-300 flex flex-col items-center">
      ...
      <div className="h-full flex flex-col gap-y-10 mt-10">
        {kudos.map(kudo => (
          <div className="h-24 w-24 relative" key={kudo.profile.userId}>
            <UserCircle profile={kudo.profile} className="w-20 h-20" />
